...
class CartService:
    ...
    def add_to_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        product = self.product_repository.get_by_id(item.product_id) # определяем товар, который добавляется
        if not product: # обработка ошибок
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product with id {item.product_id} not found"
            )

        if item.product_id in cart_data: # если этот товар уже лежит в корзине
            cart_data[item.product_id] += item.quantity # увеличиваем количество этого товара
        else:
    ...
    def update_cart_item(self, cart_data: Dict[int, int], item: CartItemUpdate) -> Dict[int, int]:
        if item.product_id not in cart_data: # обработка ошибок
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product with id {item.product_id} not found in cart"
            )

        cart_data[item.product_id] = item.quantity # изменяем количество товара в корзине
        return cart_data

    def remove_from_cart(self, cart_data: Dict[int, int], product_id: int) -> Dict[int, int]:
        if product_id not in cart_data: # обработка ошибок
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product with id {product_id} not found in cart"
            )

        del cart_data[product_id] # удаляем товар из корзины
        return cart_data
    ...
    def get_cart_details(self, cart_data: Dict[int, int]) -> CartResponse:
        ...
        total_items = 0 # общее количество единиц товара

        for product_id, quantity in cart_data.items():
            if product_id in products_dict:
                product = products_dict[product_id] # берём инфу из карточек товаров
